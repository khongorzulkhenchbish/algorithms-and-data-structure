...
|424. Longest Repeating Character Replacement|String, Hash Table, Sliding Window|Medium|Expand as much posible to the right, update left only when the sublength-maxFreqChar==k|
...
|323. Number of Connected Components in an Undirected Graph|Array, DFS, Union Find|Medium|With dfs, paint the nodes and add into the painted nodes. If you see not painted node, then increment the count. Union find is in the code.|
...
|684. Redundant Connection|Union find, Graph|Medium|The intuition is to form the tree edge by edge only if the 2 nodes have different parent. If they have same parent, this means, current nodes creates the last extra edge.|
...
|72. Edit distance|String, Dynamic Programming|Medium->Hard|Use bottom up table. If the chars are equal, we go forward, if not we should insert|delete|replace. TODO: Need to revise!|
...
|1293. Shortest Path in a Grid with Obstacles Elimination|Array, BFS|Hard (but medium)|BFS will find the shortest path fastest. Start iterating level by level, keep the visited set of path as (row, col, cur_k) as we might visit the same cell again. When visiting the cell, check the neighbors, register them immediately as visited, and add them as children to the queue.|
|1300. Sum of Mutated Array Closest to Target|Array, Binary Search, Sort|Medium|Sort first. Then the sum of mutated array will be monotonic increasing as x increases. For each x, we can calculate the mutated sum and compare it with the target. Depending on the condition, we can inc or dec the x.|
...
|127. Word Ladder|String, BFS|Hard|Use q to store the word chain with how many steps it took to get to the current word. the first word matches with the end word will be the shortest path.|
...
|1011. Capacity To Ship Packages Within D Days|Array, Binary Search|Medium|Lower bound is the max val in the arr when days=length_arr, upper bound is the sum of all elements if the days=1, run binary search to check if we can deliver with capacity mid|
...
|523. Continuous Subarray Sum|Hash Table, Prefix sum, Math|Medium-Hard|The remainder of the prefix sum being the same means between the positions the subarray exist sch that element sum is miltiplication of K|
