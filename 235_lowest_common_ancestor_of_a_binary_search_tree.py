...
class Solution:
    def lowestCommonAncestor(self, root: 'TreeNode', p: 'TreeNode', q: 'TreeNode') -> 'TreeNode':
        ...
        '''
        # Iterative approach
        # space: O(1) superior on larger input
        cur = root
        
        while cur:
            cv, pv, qv = cur.val, p.val, q.val
            
            if cv < pv and cv < qv:
                cur = cur.right
            elif cv > pv and cv > qv:
                cur = cur.left
            else:
                return cur
        ...
        # Space: O(log n) only stores half times the nodes
        if not root:
            return None
        ...
        elif root.val >= p.val and root.val >= q.val: # LCA is on the left side
            return self.lowestCommonAncestor(root.left, p, q)
        
        else: # root.val >= p.val and root.val <= q.val: # LCA is the root itself
            return root
